...
| E3 | Hardcoded `JWT_SECRET` fallback | `apps/api/src/auth/auth.module.ts` + `jwt.strategy.ts` (`JWT_SECRET \|\| <fallback>`) | If `JWT_SECRET` ever fails to load, tokens are signed/verified with a public fallback → forgeable admin tokens. Prod has the secret set → latent today | Remove the fallback; **throw** if `JWT_SECRET` is missing (fail closed) | Code | ✅ SHIPPED Deploy 47 `515a2f2` |
...
- [x] Add jest + supertest + `@nestjs/testing`; wire `pnpm test` + a GitHub Action. — ✅ 2026-06-23 (`9fe8dc1`): harness at `apps/api/test`, run `pnpm --filter @ascure/api test`. GH Action committed but **not yet validated by a real CI run**.
- [x] Write the authz matrix skeleton + the first endpoints' cross-company negatives. — ✅ 2026-06-23 (`9fe8dc1`): site-visits / inspections / defects detail → 404 cross-company; lists + dashboard + `/assets/map` scoped; cross-tenant 404; role gates (teams/users + the Deploy-44 team-org lock). **Remaining:** the rest of the endpoint matrix + scope-helper unit tests. NOTE: `assets` detail/list are **tenant-wide by design** → left as an `it.todo` decision, not asserted.
- [x] Add `@nestjs/throttler` to `/auth/login`. — ✅ SHIPPED Deploy 47 (`515a2f2`, 2026-06-23). **Still open:** `helmet` security headers + tighten CORS from `*`.
...
- **2026-06-23** — **E1 (login throttle) + E3 (JWT fail-closed) SHIPPED** to prod (Deploy 47 `515a2f2`, api-only, +`@nestjs/throttler`; nginx XFF confirmed for per-IP keying). **Phase 0/1 first slice landed** (commit `9fe8dc1`, committed not pushed): the repo's first automated tests — jest + supertest + `@nestjs/testing` harness, deterministic multi-company seed (`apps/api/test`), a GitHub Actions workflow, and **22 passing authz tests** covering cross-company 404 isolation (site-visits/inspections/defects), scoped lists/dashboard/`/assets/map`, cross-tenant, and the role/privilege gates incl. the Deploy-44 team-org regression lock. The suite runs against a **separate local `ascure_test` DB** with a hard `current_database()`-must-contain-"test" reset guard (added after a first-run env mishap truncated the local seed/demo `ascure` DB — restored, prod never involved). **Open decision:** `GET /assets/:id` + `GET /assets` (master-data) are tenant-wide — any company can read any asset by id; only `/assets/map` + site-visits/inspections/defects/dashboard are org-scoped. Confirm intended (poles are the utility's shared asset register) or scope to org. **Remaining in Phase 0/1:** rest of the endpoint matrix, scope-helper unit tests, the `RELEASE_ON_REPORT` defect-mode variant, nightly `pg_dump` + restore rehearsal, and a real first CI run.
- **2026-06-23 (cont.)** — authz matrix expanded to **73 tests** (10 unit + 63 e2e, 8 suites): cross-company read+mutation isolation, lifecycle R4 gates, ADMIN-only tenant-config gates, management list scoping, operational-sessions (company+QA scope), maintenance-handoff routed-pool visibility (the RELEASE_ON_REPORT cross-company path), + the `siteVisitAccessWhere` unit matrix. Every distinct authorization mechanism is now covered. Pushed (`515a2f2..46101a4`); the GitHub Action now also runs on the feature branch + manual dispatch, and the first real CI run was triggered. Remaining (low value): `reports/*` + `network` reads (likely tenant-wide like the asset register), `buildScopeContext` DB-backed units, and the backup/restore rehearsal.
